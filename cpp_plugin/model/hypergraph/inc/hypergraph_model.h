...
protected:
    ...
    SimplexList create_interaction_simplices_from_connections(
...
private:
    RectangleList create_transformed_filled_rectangles(const PointList &points, const float exponent) const;
    RectangleList create_rectangles(const PointList &points_in, const float exponent) const;
    ConnectionList calc_connected_point_pairs(
        const Rectangle &vertex_rectangle,
        const Rectangle &interaction_rectangle) const;

    bool rectangle_points_may_connect(const Rectangle &vertex_rectangle, const Rectangle &interaction_rectangle) const;
