...
class FiniteNetwork(Network):
    """Base class representing a finite network."""

    def __init__(self, cpp_network: CppFiniteCliqueComplex | CppFiniteHypergraph) -> None:
        """Construct an empty network."""
        super().__init__(cpp_network)
        self._components: list[FiniteNetwork] | None = None

    @log_function_name
    def get_component(self, component_index: int) -> Self:
        """Return the network of the specified component."""
        if component_index == -1:
            return self
        vertices_in_component = self._get_vertices_in_components()[component_index]
        return self._construct_self_from_cpp(self.cpp_network.filter(vertices_in_component))
    ...
    def info(self) -> dict[str, Any]:
        ...
        return super().info() | {
            'num_of_components': self.calc_base_property(BaseNetworkProperty.betti_numbers)[0],
    ...
    def get_partition(
        ...
        for vertices_in_component in vertices_in_components:
            ...
            if len(vertices_in_part) >= min_vertices_in_part:
                partitions.append(self._construct_self_from_cpp(self.cpp_network.filter(vertices_in_part)))
                vertices_in_part = []
    ...
    def _construct_self_from_cpp(self, cpp_network: CppFiniteCliqueComplex | CppFiniteHypergraph) -> Self:
